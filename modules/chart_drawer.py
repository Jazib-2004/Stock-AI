...
def get_update_interval(strategy_config_file):
    try:
        with open(strategy_config_file, 'r') as f:
            ...
            return interval_map.get(config.get('time_interval', '1min'), 60)
...
def update_chart_and_indicators(chart, rsi_line, macd_line, macd_signal_line, macd_histogram_series, stochrsi_k_line, stochrsi_d_line, ohlcv_file, strategy_config_file="strategy_config.json", config_update_event=None):
    while True:
        # Check for configuration updates
        if config_update_event and config_update_event.is_set():
            config_update_event.clear()
            
        update_interval = get_update_interval(strategy_config_file)
